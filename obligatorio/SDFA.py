...
class Agent:
        ...
        for ep in tqdm(range(number_episodes), unit=" episodes"):
            ...
            for s in range(max_steps):
                self.qval_ = self.model(
                    torch.from_numpy(np.array(self.obs_processing_func(state)))
                    .unsqueeze(0)
                    .to(self.device)
                )
                # Seleccionar accion usando una política epsilon-greedy.
                self.epsilon = self.compute_epsilon(ep)
                ...
                if len(self.memory) > self.batch_size:
                    ...
                    self.state_batch = torch.cat(
                        [
                            torch.from_numpy(np.array(self.obs_processing_func(s1)))
                            .unsqueeze(0)
                            .to(self.device)
                    ...
                    self.next_state_batch = torch.cat(
                        [
                            torch.from_numpy(np.array(self.obs_processing_func(s2)))
                            .unsqueeze(0)
                            .to(self.device)
                            for (s1, a, r, d, s2) in self.minibatch
                        ]
                    ...
                    Q1 = self.model(self.state_batch).to(self.device)
                    with torch.no_grad():
                        Q2 = self.model(self.next_state_batch).to(self.device)
    ...
    def compute_epsilon(self, steps_so_far):
        return 1 - steps_so_far / self.number_episodes
